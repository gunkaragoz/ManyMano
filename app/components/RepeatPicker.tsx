...
import { CalendarClock, CalendarPlus, Check, ChevronDown, X } from "lucide-react";
...
const fieldButton =
  "w-full flex items-center justify-between gap-2 px-4 py-3 rounded-xl border border-slate-200/90 bg-white text-sm text-left focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all disabled:opacity-50 disabled:cursor-not-allowed";
      ...
      {isRange ? (
        <div className="flex flex-wrap items-end gap-3">
          ...
          <button
            ...
            className="px-3.5 py-2.5 text-xs font-semibold rounded-2xl border border-slate-200 hover:border-rose-300 hover:text-rose-600 bg-white transition-all shadow-sm inline-flex items-center gap-1"
          >
            <X className="w-3.5 h-3.5" /> Single day
          </button>
        </div>
      ) : (
        <button
          type="button"
          onClick={() => patch({ mode: "range", end: addDays(start, 2) })}
          className="px-3.5 py-1.5 text-xs font-semibold rounded-2xl border border-slate-200 hover:border-blue-400 hover:text-blue-600 bg-white transition-all shadow-sm inline-flex items-center gap-1"
        >
          <CalendarPlus className="w-3.5 h-3.5" /> More than one day
        </button>
      )}
      ...
      <div className="relative" ref={menuRef}>
        ...
        {isRange && (
          <p className="text-[11px] text-slate-500 mt-1">
            A sheet that runs several days in a row can&apos;t also repeat.
          </p>
        )}
        {menuOpen && (
          <div
          ...
          >
            {presets.map((preset) => {
              const active = value.mode !== "repeat" ? preset.key === "none" : value.repeatKey === preset.key;
              return (
                <button
                  key={preset.key}
                  type="button"
                  ...
                  onClick={() => {
                    setMenuOpen(false);
                    if (preset.key === "none") patch({ mode: "single", repeatKey: "none" });
                    else patch({ mode: "repeat", repeatKey: preset.key });
                  }}
                  className="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-sm text-left hover:bg-slate-50 transition-colors"
                >
                  <Check className={`w-4 h-4 ${active ? "text-blue-600" : "text-transparent"}`} />
                  {preset.label}
                </button>
              );
            })}
            ...
            >
              <Check
                className={`w-4 h-4 ${value.repeatKey === "custom" && value.mode === "repeat" ? "text-blue-600" : "text-transparent"}`}
              />
              Custom…
            </button>
          </div>
